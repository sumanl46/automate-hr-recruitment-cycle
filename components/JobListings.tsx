			...
			<div className="relative mt-10">
					...
					{jobListings
						...
						.map((job, index) => (
							<Link key={job.id} href={`/jobs/${job.id}`} className="block">
								<motion.div
									className="bg-white border rounded-lg p-6 transition-shadow hover:shadow-lg"
									initial={{ opacity: 0, y: 20 }}
									animate={{ opacity: 1, y: 0 }}
								...
								>
									...
									<div className="text-xl text-black">{job.title}</div>

									<div className="text-gray-500 text-sm font-medium">{job.company}</div>

									<div className="text-sm text-gray-700 mt-4">{job.location}</div>
								</motion.div>
							</Link>
						))}
				</div>
				...
				<div className="relative w-1/3">
					<div className="relative h-auto border p-5 rounded-xl w-full">
						<div className="relative text-xl text-gray-800 font-black">Related Searches</div>
